...
impl<R: Read> XzReader<R> {
    ...
    ///
    /// `mem_limit_kb` is in KiB; `u32::MAX` disables the limit. Reading a block
    /// returns an out of memory error before dictionary allocation if its
    /// dictionary and estimated LZMA2 decoder overhead exceed the limit.
    /// Index records and additional filter buffers are excluded.
